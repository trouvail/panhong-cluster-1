...
// zhou:这个函数用于等待子进程的退出状态，如果已经通过其他方式处理了子进程的状态信息
// （通过 remember_children()），它会成功返回子进程ID和状态，否则，它会返回 
// waitpid 的结果。
...
// 这个函数用于等待子进程的退出，并根据子进程的退出状态来设置错误代码
...
// 将这些统计数据以一种特定的格式写入文件f，以供后续处理或分析使用
...
// 目的是以指定的用户和组的身份运行当前进程，并处理了相关的错误情况。
...
// 这个函数主要用于处理程序中的统计信息，根据不同的运行情况采取不同的操作，如显示统计信息或者读取/写入统计信息
...
// 为了提供有关文件传输的详细统计信息，用于在程序运行过程中向用户提供相关数据
...
// 这段代码是一个C语言函数，其目的是确定文件传输的模式和本地文件名。主要包括两种模式：
// 接收多个文件并将它们保存在目标目录中，按照文件列表中的文件名进行保存。
// 接收单个文件并将其保存在目标路径中，而不是使用文件列表中的文件名。
...
// 这段代码用于检查备用基础目录，确保它们存在且是目录，并在必要时对路径进行调整以适应干预模式。
...
// 作为服务器端的发送者，用于在文件传输过程中发送文件或文件列表
...
// 用于文件传输的一部分，实现了接收文件和生成文件的功能，同时处理了一些错误和统计信息
...
// 主要用于处理服务器端的文件接收逻辑，包括接收文件列表、目录切换、路径处理等操作
...
//  服务器的初始化和启动过程，根据不同的条件和配置选择不同的路径执行，用于处理输入和输出，以及根据协议版本执行相应的操作。
...
// 它涉及非阻塞I/O设置、协议初始化、文件传输等功能，具体根据是发送端（am_sender）还是接收端执行不同的逻辑。函数最终返回两个退出码的较大值。
...
// 这段代码的主要目的是定义程序在崩溃时应采取的默认处理动作，但允许用户通过环境变量自定义该处理动作。如果没有自定义处理动作，它会调用 GDB（GNU Debugger）来尝试捕获程序崩溃的信息。
/**
 ...
 **/
这段代码是一个信号处理函数，用于根据配置启动调试器，并执行特定命令以处理程序的致命错误。如果启动调试器成功，程序将继续运行，否则将以特定退出码终止。
